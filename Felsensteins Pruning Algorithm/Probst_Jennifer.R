...
calculate_likelihood_from_subtree_likelihoods = function(N, Q,
    ...
    for (i in 1:N){ #iterate through the sequence
      for (j in 1:4){ #iterate through the nulceotides
        likelihood_per_site[i,j] <- sum(subtree1_likelihood[i,]*P1[j,])*sum(subtree2_likelihood[i,]*P2[j,])
      }
    }
...
get_likelihood_from_sequence = function(N, sequence) {
    ...
    likelihood_per_site <- matrix(0, nrow=N, ncol=4)
    for (i in 1:N){
      b <- c(sequence[i]=='T',sequence[i]=='C',sequence[i]=='A',sequence[i]=='G')
...
get_subtree_likelihood = function(node, tree, sequences, Q) {
    ...
    if (node <= length(tree$tip.label)) {
      ...
      bl1 <- get_node_children(node, tree)$branch_length1
      bl2 <- get_node_children(node, tree)$branch_length2
      likelihood_per_site <- calculate_likelihood_from_subtree_likelihoods(N, Q, get_subtree_likelihood(child1, tree, sequences, Q), bl1,
                                                                           get_subtree_likelihood(child2, tree, sequences, Q), bl2)
    }
...
Felstensteins_pruning_loglikelihood = function(pi, alpha1, alpha2, beta, newick_tree, sequences) {
    ...
    #get log likelihoods of each site
    logs <- log(likelihood_per_site %*% pi)
    
    # Sum up the log likelihoods of each site
    loglikelihood <- sum(logs)
