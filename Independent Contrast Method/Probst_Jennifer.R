...
linear_regression = function(values1, values2) {
    ...
    lm1 <- lm(y ~ x, data.frame(x = values1, y = values2))
    sum <- summary(lm1)
    slope <- sum$coefficients[2,1]
    intercept <- sum$coefficients[1,1]
    p.value <- sum$coefficients[2,4]
    R2 <- sum$r.squared
...
calculate_trait_at_internal_node = function(trait_child1, trait_child2, branch_length_to_child1, branch_length_to_child2) {
    ...
    trait_at_internal_node <- trait_child1*(branch_length_to_child2/(branch_length_to_child1+branch_length_to_child2)) + 
                              trait_child2*(branch_length_to_child1/(branch_length_to_child1+branch_length_to_child2))
...
get_contrasts_in_subtree = function(node, tree, description) {
    ...
    updated_description <- get_contrasts_in_subtree(child1, tree, description)
    updated_description <- get_contrasts_in_subtree(child2, tree, updated_description)
    
    #save traits, contrasts, lengths
    traits <- updated_description$traits
    normalized_contrasts <- updated_description$normalized_contrasts
    branch_lengths <- updated_description$corrected_branch_lengths
    
    # Update the branch length leading to the given node, save it in the description
    branch_lengths[node] <- calculate_corrected_branch_length(node, child1, child2, branch_lengths)
    updated_description$corrected_branch_lengths <- branch_lengths
      
    # Compute the trait values and normalized contrasts for both traits, save the values in the description
    traits[node,1] <- calculate_trait_and_contrast(child1, child2, traits[,1], branch_lengths)$trait_value
    traits[node,2] <- calculate_trait_and_contrast(child1, child2, traits[,2], branch_lengths)$trait_value
    normalized_contrasts[node,1] <- calculate_trait_and_contrast(child1, child2, traits[,1], branch_lengths)$contrast
    normalized_contrasts[node,2] <- calculate_trait_and_contrast(child1, child2, traits[,2], branch_lengths)$contrast
    
    updated_description$traits <- traits
    updated_description$normalized_contrasts <- normalized_contrasts
...
get_trait_correlation = function(newick_tree, traits_at_tip) {	
    ...
    rootoftree <- get_root(tree)
    updated_description <- get_contrasts_in_subtree(rootoftree, tree, description)
    ...
    contrasts_regression <- linear_regression(updated_description$normalized_contrasts[,1], updated_description$normalized_contrasts[,2])
